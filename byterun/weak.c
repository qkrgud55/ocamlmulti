...
CAMLprim value caml_weak_create_r (pctxt ctx, value len)
{
  mlsize_t size, i;
  value res;
...
CAMLprim value caml_weak_set_r (pctxt ctx, value ar, value n, value el)
{
  mlsize_t offset = Long_val (n) + 1;
                                                   Assert (Is_in_heap (ar));
  if (offset < 1 || offset >= Wosize_val (ar)){
...
CAMLprim value caml_weak_get_r (pctxt ctx, value ar, value n)
{
  CAMLparam2 (ar, n);
  mlsize_t offset = Long_val (n) + 1;
  CAMLlocal2 (res, elt);
...
CAMLprim value caml_weak_get_copy_r (pctxt ctx, value ar, value n)
{
  CAMLparam2 (ar, n);
  mlsize_t offset = Long_val (n) + 1;
  CAMLlocal2 (res, elt);
...
CAMLprim value caml_weak_check_r (pctxt ctx, value ar, value n)
{
  mlsize_t offset = Long_val (n) + 1;
                                                   Assert (Is_in_heap (ar));
  if (offset < 1 || offset >= Wosize_val (ar)){
...
CAMLprim value caml_weak_blit_r (pctxt ctx, value ars, value ofs,
                               value ard, value ofd, value len)
{
  mlsize_t offset_s = Long_val (ofs) + 1;
  mlsize_t offset_d = Long_val (ofd) + 1;
