...
/* Free the compare stack if needed */
static void compare_free_stack_r(pctxt ctx)
{
...
/* Same, then raise Out_of_memory */
static void compare_stack_overflow_r(pctxt ctx)
{
  caml_gc_message (0x04, "Stack overflow in structural comparison\n", 0);
  compare_free_stack_r(ctx);
  caml_raise_out_of_memory_r(ctx);
}

/* Grow the compare stack */
static struct compare_item * compare_resize_stack_r(pctxt ctx, struct compare_item * sp)
{
  asize_t newsize = 2 * (ctx->compare_stack_limit - ctx->compare_stack);
  asize_t sp_offset = sp - ctx->compare_stack;
  struct compare_item * newstack;

  if (newsize >= COMPARE_STACK_MAX_SIZE) compare_stack_overflow_r(ctx);
  if (ctx->compare_stack == ctx->compare_stack_init) {
    newstack = malloc(sizeof(struct compare_item) * newsize);
    if (newstack == NULL) compare_stack_overflow_r(ctx);
...
static intnat compare_val_r(pctxt ctx, value v1, value v2, int total)
{
  struct compare_item * sp;
  tag_t t1, t2;
  ...
  while (1) {
    if (v1 == v2 && total) goto next_item_r;
    if (Is_long(v1)) {
      if (v1 == v2) goto next_item_r;
      if (Is_long(v2))
        return Long_val(v1) - Long_val(v2);
      /* Subtraction above cannot overflow and cannot result in UNORDERED */
      if (Is_in_value_area(v2)) {
    ...
    default: {
      ...
      if (sz1 > 1) {
        sp++;
        if (sp >= ctx->compare_stack_limit) sp = compare_resize_stack_r(ctx, sp);
        sp->v1 = &Field(v1, 1);
        sp->v2 = &Field(v2, 1);
        sp->count = sz1 - 1;
      }
...
CAMLprim value caml_compare_r(pctxt ctx, value v1, value v2)
{
  intnat res = compare_val_r(ctx, v1, v2, 1);
  /* Free stack if needed */
  if (ctx->compare_stack != ctx->compare_stack_init) compare_free_stack_r(ctx);
  if (res < 0)
    return Val_int(LESS);
  else if (res > 0)
    return Val_int(GREATER);
...
CAMLprim value caml_equal_r(pctxt ctx, value v1, value v2)
{
  intnat res = compare_val_r(ctx, v1, v2, 0);
  if (ctx->compare_stack != ctx->compare_stack_init) compare_free_stack_r(ctx);
  return Val_int(res == 0);
}

CAMLprim value caml_notequal_r(pctxt ctx, value v1, value v2)
{
  intnat res = compare_val_r(ctx, v1, v2, 0);
  if (ctx->compare_stack != ctx->compare_stack_init) compare_free_stack_r(ctx);
  return Val_int(res != 0);
...
CAMLprim value caml_lessthan_r(pctxt ctx, value v1, value v2)
{
  intnat res = compare_val_r(ctx, v1, v2, 0);
  if (ctx->compare_stack != ctx->compare_stack_init) compare_free_stack_r(ctx);
  return Val_int(res < 0 && res != UNORDERED);
...
CAMLprim value caml_lessequal_r(pctxt ctx, value v1, value v2)
{
  intnat res = compare_val_r(ctx, v1, v2, 0);
  if (ctx->compare_stack != ctx->compare_stack_init) compare_free_stack_r(ctx);
  return Val_int(res <= 0 && res != UNORDERED);
...
CAMLprim value caml_greaterthan_r(pctxt ctx, value v1, value v2)
{
  intnat res = compare_val_r(ctx, v1, v2, 0);
  if (ctx->compare_stack != ctx->compare_stack_init) compare_free_stack_r(ctx);
  return Val_int(res > 0);
...
CAMLprim value caml_greaterequal_r(pctxt ctx, value v1, value v2)
{
  intnat res = compare_val_r(ctx, v1, v2, 0);
  if (ctx->compare_stack != ctx->compare_stack_init) compare_free_stack_r(ctx);
  return Val_int(res >= 0);
