...
/* Function application */
    ...
    Instruct(PUSH_RETADDR): {
		if (is_ctx) printf("PUSH_RETADDR accu=%p ", accu);
      sp -= 3;
      sp[0] = (value) (pc + *pc);
      sp[1] = env;
    ...
    Instruct(APPLY): {
		if (is_ctx) printf("APPLY accu=%p ", accu);
      extra_args = *pc - 1;
      pc = Code_val(accu);
      env = accu;
      goto check_stacks;
    ...
    Instruct(APPLY1): {
		if (is_ctx) printf("APPLY1 accu=%p ", accu);
      value arg1 = sp[0];
      sp -= 3;
      sp[0] = arg1;
      sp[1] = (value)pc;
    ...
    Instruct(APPLY2): {
		if (is_ctx) printf("APPLY2 accu=%p ", accu);
      value arg1 = sp[0];
      value arg2 = sp[1];
      sp -= 3;
      sp[0] = arg1;
    ...
    Instruct(APPLY3): {
		if (is_ctx) printf("APPLY3 accu=%p ", accu);
      value arg1 = sp[0];
      value arg2 = sp[1];
      value arg3 = sp[2];
      sp -= 3;
    ...
    Instruct(APPTERM): {
		if (is_ctx) printf("APPTERM accu=%p ", accu);
      int nargs = *pc++;
      int slotsize = *pc;
      value * newsp;
      int i;
    ...
    Instruct(APPTERM1): {
		if (is_ctx) printf("APPTERM1 accu=%p ", accu);
      value arg1 = sp[0];
      sp = sp + *pc - 1;
      sp[0] = arg1;
      pc = Code_val(accu);
    ...
    Instruct(APPTERM2): {
		if (is_ctx) printf("APPTERM2 accu=%p ", accu);
      value arg1 = sp[0];
      value arg2 = sp[1];
      sp = sp + *pc - 2;
      sp[0] = arg1;
    ...
    Instruct(APPTERM3): {
		if (is_ctx) printf("APPTERM3 accu=%p ", accu);
      value arg1 = sp[0];
      value arg2 = sp[1];
      value arg3 = sp[2];
      sp = sp + *pc - 3;
    ...
    Instruct(RETURN): {
		if (is_ctx) printf("RETURN accu=%p ", accu);
      sp += *pc++;
      if (extra_args > 0) {
        extra_args--;
        pc = Code_val(accu);
    ...
    Instruct(RESTART): {
		if (is_ctx) printf("RESTART accu=%p ", accu);
    ...
    Instruct(GRAB): {
		if (is_ctx) printf("GRAB accu=%p ", accu);
      int required = *pc++;
    ...
    Instruct(CLOSURE): {
		if (is_ctx) printf("CLOSURE accu=%p ", accu);
      int nvars = *pc++;
      int i;
      if (nvars > 0) *--sp = accu;
    ...
    Instruct(CLOSUREREC): {
		if (is_ctx) printf("CLOSUREREC accu=%p ", accu);
      int nfuncs = *pc++;
      int nvars = *pc++;
      int i;
      value * p;
...
/* Access to global variables */
    ...
    Instruct(GETGLOBALFIELD): {
		if (is_ctx) printf("GETGLOBALFIELD accu=%p ", accu);
      accu = Field(caml_global_data, *pc);
      pc++;
      accu = Field(accu, *pc);
      pc++;
...
/* Allocation of blocks */
    ...
    Instruct(MAKEBLOCK): {
		if (is_ctx) printf("MAKEBLOCK accu=%p ", accu);
      mlsize_t wosize = *pc++;
      tag_t tag = *pc++;
      mlsize_t i;
      value block;
    ...
    Instruct(MAKEBLOCK1): {
		if (is_ctx) printf("MAKEBLOCK1 accu=%p ", accu);
      tag_t tag = *pc++;
      value block;
    ...
    Instruct(MAKEBLOCK2): {
		if (is_ctx) printf("MAKEBLOCK2 accu=%p ", accu);
      tag_t tag = *pc++;
      value block;
    ...
    Instruct(MAKEBLOCK3): {
		if (is_ctx) printf("MAKEBLOCK3 accu=%p ", accu);
      tag_t tag = *pc++;
      value block;
    ...
    Instruct(MAKEFLOATBLOCK): {
		if (is_ctx) printf("MAKEFLOATBLOCK accu=%p ", accu);
      mlsize_t size = *pc++;
      mlsize_t i;
      value block;
...
/* Access to components of blocks */
    ...
    Instruct(GETFLOATFIELD): {
		if (is_ctx) printf("GETFLOATFIELD accu=%p ", accu);
...
/* Array operations */
    ...
    Instruct(VECTLENGTH): {
		if (is_ctx) printf("VECTLENGTH accu=%p ", accu);
...
/* Branches and conditional branches */
    ...
    Instruct(BRANCHIFNOT):
      ...
      Next;
    Instruct(SWITCH): {
		if (is_ctx) printf("SWITCH accu=%p ", accu);
      uint32 sizes = *pc++;
      if (Is_block(accu)) {
...
/* Calling C functions */
    ...
    Instruct(C_CALL5):
      ...
      Next;
    Instruct(C_CALLN): {
		if (is_ctx) printf("C_CALLN accu=%p ", accu);
      int nargs = *pc++;
      *--sp = accu;
      Setup_for_c_call;
    ...
    Instruct(C_CALL5_R):
      ...
      Next;
    Instruct(C_CALLN_R): {
		if (is_ctx) printf("C_CALLN_R accu=%p ", accu);
      int nargs = *pc++;
      accu = ctx;
      *--sp = accu;
      Setup_for_c_call;
...
/* Integer arithmetic */
    ...
    Instruct(DIVINT): {
		if (is_ctx) printf("DIVINT accu=%p ", accu);
    ...
    Instruct(MODINT): {
		if (is_ctx) printf("MODINT accu=%p ", accu);
      intnat divisor = Long_val(*sp++);
      if (divisor == 0) { Setup_for_c_call; caml_raise_zero_divide(); }
#ifdef NONSTANDARD_DIV_MOD
...
#define CAML_METHOD_CACHE
    ...
    Instruct(GETPUBMET): {
		if (is_ctx) printf("GETPUBMET accu=%p ", accu);
...
#endif
    Instruct(GETDYNMET): {
		if (is_ctx) printf("GETDYNMET accu=%p ", accu);
