...
// phc - sentinel always remains as the first free block
...
/* [allocate_block] is called by [caml_fl_allocate].  Given a suitable free
   ...
   the linking of the free-list does not change in case 2.
*/
static char *allocate_block_r (pctxt ctx, mlsize_t wh_sz, int flpi, char *prev, char *cur)
{
  header_t h = Hd_bp (cur);
                                             Assert (Whsize_hd (h) >= wh_sz);
  if (Wosize_hd (h) < wh_sz + 1){                        /* Cases 0 and 1. */
    // phc - take the block out of the freelist chain
    ...
    if (ctx->caml_fl_merge == cur) ctx->caml_fl_merge = prev;
      ...
      /* In case 1, the following creates the empty block correctly.
         In case 0, it gives an invalid header to the block.  The function
         calling [caml_fl_allocate] will overwrite it. */
    // phc - in case 0, we ignore the following line of code
    Hd_op (cur) = Make_header (0, 0, Caml_white);

    if (policy == Policy_first_fit){
      // phc - Assumption : Next(flp[flpi])==cur
      //   flp[flpi+1]==cur  implies that 
      //     Next(flp[flpi])==flp[flpi+1] (adjacent in flp)
      if (flpi + 1 < ctx->flp_size && ctx->flp[flpi + 1] == cur){
        // phc - prev is duplicate in flp which is no harm
        ctx->flp[flpi + 1] = prev;
      }else if (flpi == ctx->flp_size - 1){
        ctx->beyond = (prev == ctx->fl_head) ? NULL : prev;
        -- ctx->flp_size;
      }
    }
  }else{                                                        /* Case 2. */
    ctx->caml_fl_cur_size -= wh_sz;
    // phc - keep the front part of the block in the free list
    Hd_op (cur) = Make_header (Wosize_hd (h) - wh_sz, 0, Caml_blue);
  }

  if (policy == Policy_next_fit) ctx->fl_prev = prev;

  // phc - return the rear part of the block
  return cur + Bosize_hd (h) - Bsize_wsize (wh_sz);
}
...
/* [caml_fl_allocate] does not set the header of the newly allocated block.
   ...
   [caml_fl_allocate] returns a head pointer.
*/
char *caml_fl_allocate_r (pctxt ctx, mlsize_t wo_sz)
{
  char *cur = NULL, *prev, *result;
  int i;
  mlsize_t sz, prevsz;
  ...
  case Policy_next_fit:
    ...
    // phc - Policy_next_fit : simple round robin until we find 
    ...
    while (cur != NULL){                             Assert (Is_in_heap (cur));
      if (Wosize_bp (cur) >= wo_sz){
        // phc - allocate_block take the block(cur) out of the free list chain
        return allocate_block_r (ctx, Whsize_wosize (wo_sz), 0, prev, cur);
      }
      prev = cur;
      cur = Next (prev);
    }
  ...
  case Policy_first_fit: {
    // phc - Policry_first_fit : maintain a cache chain(flp)
    //         blocks in the flp chain are in the increasing order of
    //           both pointer address and block size
    //       Actually flp keeps the prev block of those blocks maintained.
    //       Policy_first_fit improves search speed since we only follow flp
    //
    ...
    for (i = 0; i < ctx->flp_size; i++){
      ...
      if (sz >= wo_sz){
        result = allocate_block_r (ctx, Whsize_wosize (wo_sz), 
                                   i, ctx->flp[i], Next(ctx->flp[i]));
        goto update_flp_r;
      }
    }
    /* If not found, extend the flp array. */
    if (ctx->flp_size == 0){ 
      // phc - empty flp, init with fl_head(sentinel)
      prev = ctx->fl_head;
      prevsz = 0;
    }else{
      // phc - pick the last one of flp
      prev = Next (ctx->flp[ctx->flp_size - 1]);
      prevsz = Wosize_bp (prev);
      // phc - beyond is the block we have checked in the last extension
      //         we may start from beyond
    ...
    while (ctx->flp_size < FLP_MAX){
      ...
      if (cur == NULL){
        // phc - we reached the end of freelist
        ctx->fl_last = prev;
        ctx->beyond = (prev==ctx->fl_head) ? NULL : prev;
        // phc - nothing to extend, we are still in failure of allocation
        return NULL;
      }else{
        sz = Wosize_bp (cur);
        if (sz > prevsz){
          // phc - we found one that we can attach to the end of flp
          ctx->flp[ctx->flp_size] = prev;
          ++ ctx->flp_size;

          if (sz >= wo_sz){
            // phc - the new one is for allocation.
            //   Allocate right after extension
            ctx->beyond = cur;
            i = ctx->flp_size - 1;
            result = allocate_block_r (ctx, Whsize_wosize (wo_sz), 
                                       ctx->flp_size - 1, prev, cur);
            goto update_flp_r;
          }
          prevsz = sz;
        }
    ...
    // phc - we keep the beyond according to its semantics 
    ctx->beyond = cur;
    ...
    while (cur != NULL){
      ...
      sz = Wosize_bp (cur);
      if (sz < prevsz){
        // phc - we keep updating beyond in order to save 
        //   saerch tests next time
        ctx->beyond = cur;
      }else if (sz >= wo_sz){
        // phc - no update of flp (no update_flp_r)
        return allocate_block_r (ctx, Whsize_wosize (wo_sz), 
                                 ctx->flp_size, prev, cur);
      }
      prev = cur;
      cur = Next (prev);
    }
  ...
  update_flp_r: /* (i, sz) */
    ...
    if (i < ctx->flp_size){
      ...
      if (i == ctx->flp_size - 1){
        // phc - found block is the last in flp
        if (Wosize_bp (Next (ctx->flp[i])) <= prevsz){
          // phc - the second last one(prevsz) has equal or greater size
          //       start from the last one when searching next time(beyond)
          ctx->beyond = Next (ctx->flp[i]);
          -- ctx->flp_size;
        }else{
          // start again from the second last one when searching next time
          ctx->beyond = NULL;
        }
      }else{
        // phc - remove a block in the middle of flp
        char *buf [FLP_MAX];
        int j = 0;
        mlsize_t oldsz = sz;

        // phc - prev of the found one
        //   follow until the next flp (flp[i+1]) 
        prev = ctx->flp[i];
        while (prev != ctx->flp[i+1]){
          cur = Next (prev);
          sz = Wosize_bp (cur);
          if (sz > prevsz){
            // phc - store new blocks for flp between (i-1) and (i+1)
            buf[j++] = prev;
            prevsz = sz;
            if (sz >= oldsz){
              // phc - impossbile to have bigger block than (i)
              //   which was not registered in flp 
              Assert (sz == oldsz);
              break;
            }
          }
        ...
        // phc - shifting flp array because we removed one in the middle
        //       and we have buf(len==j) to insert in the middle
        if (FLP_MAX >= ctx->flp_size + j - 1){
          if (j != 1){
            memmove (&(ctx->flp[i+j]), 
                     &(ctx->flp[i+1]), 
                     sizeof (block *) * (ctx->flp_size-i-1));
          }
          if (j > 0) 
             memmove (&(ctx->flp[i]), &buf[0], sizeof (block *) * j);
          ctx->flp_size += j - 1;
        }else{
          if (FLP_MAX > i + j){
            if (j != 1){
              memmove (&(ctx->flp[i+j]), 
                       &(ctx->flp[i+1]), 
                       sizeof (block *) * (FLP_MAX-i-j));
            }
            if (j > 0) 
              memmove (&(ctx->flp[i]), &buf[0], sizeof (block *) * j);
          }else{
            if (i != FLP_MAX){
              memmove (&(ctx->flp[i]), &buf[0], 
                       sizeof (block *) * (FLP_MAX - i));
            }
          }
...
void caml_fl_init_merge_r (pctxt ctx)
{
  ...
  ctx->caml_fl_merge = ctx->fl_head;
}

static void truncate_flp_r (pctxt ctx, char *changed)
{
  if (changed == ctx->fl_head){
    ctx->flp_size = 0;
    ctx->beyond = NULL;
  }else{
    while (ctx->flp_size > 0 && Next (ctx->flp[ctx->flp_size - 1]) >= changed) 
      -- ctx->flp_size;
    if (ctx->beyond >= changed) 
      ctx->beyond = NULL;
  }
}

void caml_fl_reset_r (pctxt ctx)
{
  Next (ctx->fl_head) = NULL;
  switch (policy){
  case Policy_next_fit:
    ctx->fl_prev = ctx->fl_head;
    break;
  case Policy_first_fit:
    truncate_flp_r (ctx, ctx->fl_head);
    break;
  default:
    Assert (0);
    break;
...
// phc - insert a new block into freelist and flp
//   merge the block if its possible.
//   Remember 1) that the blocks given as arguement are 
//   from major GC sweeping and 2) that blocks are given in the 
//   increasing order of pointer address.
//   That's why we try to merge the block with 
//    last_fragment or Next(ctx->caml_fl_merge).
char *caml_fl_merge_block_r (pctxt ctx, char *bp)
{
  ...
  /* The sweep code makes sure that this is the right place to insert
     this block: */
  Assert (prev < bp || prev == ctx->fl_head);
  Assert (cur > bp || cur == NULL);

  if (policy == Policy_first_fit) truncate_flp_r (ctx, prev);
...
/* This is a heap extension.  We have to insert it in the right place
   ...
   the last block.
*/
void caml_fl_add_blocks_r (pctxt ctx, char *bp)
{
                                                  Assert (ctx->fl_last != NULL);
                                           Assert (Next (ctx->fl_last) == NULL);
  ctx->caml_fl_cur_size += Whsize_bp (bp);

  // phc - freelist chain is in the increasing order of address
  if (bp > ctx->fl_last){
    // phc - new block is beyond the last one of free list
    Next (ctx->fl_last) = bp;
    // phc - (caml_fl_merge < caml_gc_sweep_hp) is an invariant
    ...
    prev = ctx->fl_head;
    cur = Next (prev);
    while (cur != NULL && cur < bp){   Assert (prev < bp || prev == ctx->fl_head);
      /* XXX TODO: extend flp on the fly */
      prev = cur;
      cur = Next (prev);
    }                                  Assert (prev < bp || prev == ctx->fl_head);
    ...
    if (policy == Policy_first_fit) truncate_flp_r (ctx, bp);
  }
}
...
void caml_set_allocation_policy_r (pctxt ctx, uintnat p)
{
  switch (p){
  case Policy_next_fit:
    ctx->fl_prev = ctx->fl_head;
