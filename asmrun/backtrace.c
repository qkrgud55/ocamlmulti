...
CAMLprim value caml_record_backtrace_r(pctxt ctx, value vflag)
{
  int flag = Int_val(vflag);
  ...
  if (flag != ctx->caml_backtrace_active) {
    ...
    if (flag) {
      caml_register_global_root_r(ctx, &ctx->caml_backtrace_last_exn);
    } else {
      caml_remove_global_root_r(ctx, &ctx->caml_backtrace_last_exn);
    }
  }
  return Val_unit;
}
...
CAMLprim value caml_backtrace_status_r(pctxt ctx, value vunit)
{
...
/* Extract location information for the given frame descriptor */
static void extract_location_info_r(pctxt ctx, frame_descr * d,
                                  /*out*/ struct loc_info * li)
{
  uintnat infoptr;
  uint32 info1, info2;
...
static void print_location_r(pctxt ctx, struct loc_info * li, int index)
{
  char * info;
...
void caml_print_exception_backtrace_r(pctxt ctx)
{
  ...
  for (i = 0; i < ctx->caml_backtrace_pos; i++) {
    extract_location_info_r(ctx, (frame_descr *) (ctx->caml_backtrace_buffer[i]), &li);
    print_location_r(ctx, &li, i);
  }
}

/* Convert the backtrace to a data structure usable from OCaml */

CAMLprim value caml_get_exception_backtrace_r(pctxt ctx, value unit)
{
  CAMLparam0_r(ctx);
  CAMLlocal4_r(ctx, res, arr, p, fname);
  int i;
  struct loc_info li;

  arr = caml_alloc_r(ctx, ctx->caml_backtrace_pos, 0);
  for (i = 0; i < ctx->caml_backtrace_pos; i++) {
    extract_location_info_r(ctx, (frame_descr *) (ctx->caml_backtrace_buffer[i]), &li);
    if (li.loc_valid) {
