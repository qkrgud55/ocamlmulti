...
CAMLprim value caml_array_get_addr_r(pctxt ctx, value array, value index)
{
  intnat idx = Long_val(index);
...
CAMLprim value caml_array_get_float_r(pctxt ctx, value array, value index)
{
  intnat idx = Long_val(index);
  double d;
  value res;
...
CAMLprim value caml_array_get_r(pctxt ctx, value array, value index)
{
  if (Tag_val(array) == Double_array_tag)
...
CAMLprim value caml_array_set_addr_r(pctxt ctx, value array, value index, value newval)
{
  intnat idx = Long_val(index);
  if (idx < 0 || idx >= Wosize_val(array)) caml_array_bound_error_r(ctx);
  Modify_r(ctx, &Field(array, idx), newval);
  return Val_unit;
}

CAMLprim value caml_array_set_float_r(pctxt ctx, value array, value index, value newval)
{
  intnat idx = Long_val(index);
  if (idx < 0 || idx >= Wosize_val(array) / Double_wosize)
    caml_array_bound_error_r(ctx);
...
CAMLprim value caml_array_set_r(pctxt ctx, value array, value index, value newval)
{
  if (Tag_val(array) == Double_array_tag)
...
CAMLprim value caml_array_unsafe_get_float_r(pctxt ctx, value array, value index)
{
  double d;
  value res;
...
CAMLprim value caml_array_unsafe_get_r(pctxt ctx, value array, value index)
{
  if (Tag_val(array) == Double_array_tag)
...
CAMLprim value caml_array_unsafe_set_addr_r(pctxt ctx, value array, value index,value newval)
{
  intnat idx = Long_val(index);
  Modify_r(ctx, &Field(array, idx), newval);
  return Val_unit;
...
CAMLprim value caml_array_unsafe_set_r(pctxt ctx, value array, value index, value newval)
{
  if (Tag_val(array) == Double_array_tag)
    return caml_array_unsafe_set_float(array, index, newval);
  else
...
CAMLprim value caml_make_array_r(pctxt ctx, value init)
{
  CAMLparam1_r (ctx, init);
  mlsize_t wsize, size, i;
  CAMLlocal2_r (ctx, v, res);

  size = Wosize_val(init);
  if (size == 0) {
    CAMLreturn_r (ctx, init);
  } else {
    v = Field(init, 0);
    if (Is_long(v)
        || ! Is_in_value_area(v)
...
CAMLprim value caml_array_blit_r(pctxt ctx, value a1, value ofs1, value a2, value ofs2,
                               value n)
{
  value * src, * dst;
  intnat count;
...
static value caml_array_gather_r(pctxt ctx, intnat num_arrays,
                               value arrays[/*num_arrays*/],
                               intnat offsets[/*num_arrays*/],
                               intnat lengths[/*num_arrays*/])
{
  CAMLparamN_r(ctx, arrays, num_arrays);
  value res;                    /* no need to register it as a root */
  int isfloat;
  mlsize_t i, size, wsize, count, pos;
  value * src;
...
CAMLprim value caml_array_sub_r(pctxt ctx, value a, value ofs, value len)
{
  value arrays[1] = { a };
  intnat offsets[1] = { Long_val(ofs) };
  intnat lengths[1] = { Long_val(len) };
...
CAMLprim value caml_array_append_r(pctxt ctx, value a1, value a2)
{
  value arrays[2] = { a1, a2 };
  intnat offsets[2] = { 0, 0 };
  intnat lengths[2] = { caml_array_length(a1), caml_array_length(a2) };
...
CAMLprim value caml_array_concat_r(pctxt ctx, value al)
{
#define STATIC_SIZE 16
  value static_arrays[STATIC_SIZE], * arrays;
  intnat static_offsets[STATIC_SIZE], * offsets;
  ...
  if (n <= STATIC_SIZE) {
    ...
    arrays = caml_stat_alloc_r(ctx, n * sizeof(value));
    offsets = caml_stat_alloc_r(ctx, n * sizeof(intnat));
    lengths = caml_stat_alloc_r(ctx, n * sizeof(value));
  }
  /* Build the parameters to caml_array_gather */
  for (i = 0, l = al; l != Val_int(0); l = Field(l, 1), i++) {
    arrays[i] = Field(l, 0);
