...
// phc todo reentrant
DECLARE_SIGNAL_HANDLER(handle_signal_r)
{
#if !defined(POSIX_SIGNALS) && !defined(BSD_SIGNALS)
  signal(sig, handle_signal);
#endif
  if (sig < 0 || sig >= NSIG) return;
  if (caml_try_leave_blocking_section_hook ()) {
    caml_execute_signal_r(handler_ctx, sig, 1);
    caml_enter_blocking_section_hook();
  } else {
    caml_record_signal_r(handler_ctx, sig);
  /* Some ports cache [caml_young_limit] in a register.
     Use the signal context to modify that register too, but only if
     we are inside OCaml code (not inside C code). */
#if defined(CONTEXT_PC) && defined(CONTEXT_YOUNG_LIMIT)
    if (Is_in_code_area(CONTEXT_PC)){
      CONTEXT_YOUNG_LIMIT = (context_reg) handler_ctx->caml_young_limit;
    }
#endif
  }
}
...
int caml_set_signal_action_r(pctxt ctx, int signo, int action)
{
  ...
  case 1:
    ...
    break;
  default:
    SET_SIGACT(sigact, handle_signal_r);
    break;
  }
  sigemptyset(&sigact.sa_mask);
  if (sigaction(signo, &sigact, &oldsigact) == -1) return -1;
  ...
  if (oldact == SIG_ERR) return -1;
#endif
  if (oldact == (signal_handler) handle_signal_r)
    return 2;
  else if (oldact == SIG_IGN)
    return 1;
  else
...
// phc todo reentrant
#if defined(TARGET_power) || (defined(TARGET_sparc) && defined(SYS_solaris))
DECLARE_SIGNAL_HANDLER(trap_handler_r)
{
#if defined(SYS_solaris)
  if (info->si_code != ILL_ILLTRP) {
    /* Deactivate our exception handler and return. */
...
// phc todo reentrant
DECLARE_SIGNAL_HANDLER(segv_handler_r)
{
  struct rlimit limit;
  struct sigaction act;
  char * fault_addr;
...
// dummy pctxt. DO NOT REFERENCE IT.
void caml_init_signals_r(pctxt ctx)
{
  /* Bound-check trap handling */
#if defined(TARGET_sparc) && defined(SYS_solaris)
  { struct sigaction act;
    sigemptyset(&act.sa_mask);
    SET_SIGACT(act, trap_handler_r);
    act.sa_flags |= SA_NODEFER;
    sigaction(SIGILL, &act, NULL);
  }
#endif
